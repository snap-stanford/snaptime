...
void GeneratePrimaryDirectories(SnapTimeConfiguration config)
{
    ...
    TSParserManager manager(TStr(config.PrimaryDirectory.c_str()),
                            TStr(config.SchemaFile.c_str()), PrimaryHierarchy,
                            TInt(config.NumThreads), 500000);
...
void GenerateSymbolicIndex(SnapTimeConfiguration config)
{
    ...
    TSTimeSymDir SymDirMaker(TStr(config.PrimaryDirectory.c_str()),
                             TStr(config.SymbolicDirectory.c_str()),
                             QuerySplit, TStr(config.SchemaFile.c_str()));
...
TTimeCollection Query(SnapTimeConfiguration config, QueryObject &Query, std::string OutputFile, bool ZeroFlag)
{
    ...
    TSTimeSymDir SymDirMaker(TStr(config.PrimaryDirectory.c_str()),
                             TStr(config.SymbolicDirectory.c_str()), QuerySplit,
                             TStr(config.SchemaFile.c_str()));
