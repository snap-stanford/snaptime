...
class TSParserManager
{
  ...
  public:
	TSParserManager(TStr _OutputDir, TStr SchemaFile, TVec<int> _MHierarchy, TInt _NumThreads = 1,
					TUInt _MaxParserCapacity = TUInt::Mx) : NumThreads(_NumThreads),
															Schema(SchemaFile),
															OutputDirectory(_OutputDir),
															ModHierarchy(_MHierarchy),
															MaxParserCapacity(_MaxParserCapacity),
															filesys_lock()
	{
		// create the directory if it does not already exist
		if (!TDir::Exists(OutputDirectory))
			TDir::GenDir(OutputDirectory);
		//create parsers
		for (int i = 0; i < NumThreads; i++)
		{
			parsers.Add(TSTimeParser(_OutputDir, &Schema, _MHierarchy,
									 &filesys_lock, MaxParserCapacity));
		}
	}
