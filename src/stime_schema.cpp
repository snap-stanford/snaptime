...
bool TSchema::GetNextUncommentedLine(TFIn &instream, TStr &result)
{
    ...
    {
        if (result.Len() != 0 && !result.StartsWith("#"))
            return true;
    }
    return false;
}
                                   ...
                                   SchemaCategory &schema_category)
{
    ...
    int section_index = -1;
    // no more lines left
    if (!GetNextUncommentedLine(instream, section_hdr))
        return false;
...
{
    AssertR(!KeyNamesToIndex.IsKey(key_name),
            "key name " + key_name + " has already been used ");
...
TTime TSchema::ConvertTime(const TStr &time_val) const
{
    ...
    AssertR(strptime(secondsString.CStr(), TimeFormatter.CStr(), &ts) != NULL,
            "invalid time formatter");
    time_t t = mktime(&ts);
    double t_d = t;
